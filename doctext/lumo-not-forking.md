...
value can be one of: `patch`, `replace` or `sed`, as described below.

The following methods are provided with the current version of not-fork:

- `patch`: the final part of the file is in a format suitable for passing
as standard input to the "patch" program.
- `replace`: one or more files in the upstream must be completely replaced;
the final part of the file contains one or more lines with format
"old-file = new-file", where both are relative paths, the first relative
to the root of the extracted upstream sources; the second path is relative
to the configuration directory. The "old-file" does not need to exist,
it will be created if necessary. If the "new-file" name is prefixed with
"+", it will have variables expanded just as described in the replacement
part of `sed`.
- `sed`: this currently allows a rudimentary search and replace on
whole files, similar to `sed -e s/patter/replacement/` but with some
limitations; a future version may improve on this; the final part of
the file contains lines with format "file-glob: regular-expression =
replacement" where file-glob is a shell-like pattern, regular-expression
is a perl-like regular expression and replacement is a possibly empty
string; the regular expression needs to be enclosed in single or double
quotes if it contains spaces or equal signs. The replacement string can
refer to some variables defined by the program such as `$LUMO_VERSION`
and `$LUMO_COMMIT_ID`. A future version may include more sed-like commands
or running an existing sed program.
